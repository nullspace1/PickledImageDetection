...
class Loss(nn.Module):
    ...
    def forward(self,box : Tensor, found : Tensor, box_gt : Tensor, found_gt : Tensor) -> Tensor:
        ...
        found_loss = F.binary_cross_entropy(found, found_gt.reshape(-1,1), reduction='sum')
        return self.weight_box * box_loss + self.weight_found * found_loss    
    
    def test(self):
        print("Testing loss...")
        input_box = torch.randn(2,4)
        input_found = torch.sigmoid(torch.randn(2,1))
        input_box_gt = torch.randn(2,4)
        input_found_gt = torch.sigmoid(torch.randn(2,1))
